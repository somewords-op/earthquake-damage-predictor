...
root = tk.Tk()
root.title("🏗️ Earthquake Damage Prediction")
...
tk.Label(scrollable_frame, text=" Earthquake Damage Prediction Tool", 
         font=("Arial", 16, "bold"), bg="#f4f4f4").grid(row=0, column=0, columnspan=2, pady=10)

row = 1
for key in fields:
    tk.Label(scrollable_frame, text=key.replace("_", " ").title(), bg="#f4f4f4", anchor="w").grid(row=row, column=0, sticky="w", padx=10, pady=3)
    
    if key in options:
        cb = ttk.Combobox(scrollable_frame, textvariable=fields[key], values=options[key], state="readonly")
        cb.grid(row=row, column=1, padx=10, pady=3)
        cb.current(0)
    elif "has_superstructure" in key or key in ["condition_post_eq", "technical_solution_proposed"]:
        tk.Checkbutton(scrollable_frame, variable=fields[key], bg="#f4f4f4").grid(row=row, column=1, padx=10, pady=3)
    else:
        tk.Entry(scrollable_frame, textvariable=fields[key]).grid(row=row, column=1, padx=10, pady=3)
    row += 1
...
def predict_damage():
    try:
        ...
        for col in model_columns:
            val = fields[col].get()
            if col in options:
                # Convert dropdown to integer code
                val = int(val.split()[0])
...
tk.Button(scrollable_frame, text="Predict Damage Grade", command=predict_damage, 
          bg="#0066cc", fg="white", font=("Arial", 12, "bold"), padx=20, pady=5).grid(row=row, column=0, columnspan=2, pady=20)
